...
angular.module("vokal.timePicker", []).directive("timePicker", timePicker);

timePicker.$inject = ['$log', '$compile', '$filter', '$document', '$timeout'];
...
function timePicker($log, $compile, $filter, $document, $timeout) {
    ...
    function linkFn(scope, element, attrs, ngModelController) {
        ...
        function createLocalMoment() {
            localMoment = moment.isMoment(scope.model) ? scope.model.clone().startOf('minute') : createNow().startOf('minute');
        }
